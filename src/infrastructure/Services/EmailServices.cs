    ...
    public class EmailServices: IEmailSender
    {
        ...
        public async Task SendEmailAsync(string email, string subject, string htmlMessage){
            ...
            if(string.IsNullOrEmpty(email))
                throw new ArgumentNullException(nameof(email), "Recipient email is missing");
            if(string.IsNullOrEmpty(subject))
                throw new ArgumentNullException(nameof(subject), "Email subject is missing");
            if(string.IsNullOrEmpty(htmlMessage))
                throw new ArgumentNullException(nameof(htmlMessage), "Email body is missing");

            using (var smtpClient = new SmtpClient(_host){                 //Tạo một phiên bản của Smtp với hostname của SMTP server
                ...
                using (var mailMessage = new MailMessage{
                    From = new MailAddress(_email),                  //Định nghĩa mail người gửi
                    Subject = subject,                               //Thiết lập tiêu đề mail
                    Body = htmlMessage,                                     //Thiêt lập bidy Email
                    IsBodyHtml = true                                //Cho phép nội dung mail là HTML
                }){
                    mailMessage.To.Add(email);                          //Thêm địa chỉ email người nhận vào danh sách người nhận
                    try{
                        await smtpClient.SendMailAsync(mailMessage); //Gửi mail bất đồng bộ
                    }catch(Exception ex){
